...
function App() {
  ...
  const [deleteDetails, setDeleteDetails] = useState({index: 0});
  ...
  function deleteRule(e, i){
    e.preventDefault();
    console.log('rules from deleteRule ',rules);
    console.log('json from delete Rule ',json);
    setDeleteDetails({index: i});
    setIsDeleted(true);
  }
  ...
  useEffect(()=>{
    if(isDeleted){
      setRules(prevState => {
        prevState.filter((prev, i)=>{
          return i !== deleteDetails.index;
        })
      });
